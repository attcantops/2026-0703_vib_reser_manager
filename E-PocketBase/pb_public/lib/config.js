...
const CONFIG = {
  ...
  // 이메일은 선택이다. 적은 사람에게만 알림이 간다.
  //
  // 취소 통보(남이 내 예약을 지웠을 때 알려주는 메일)는 이메일이 있어야 갈 수
  // 있다. 그래도 필수로 강제하지 않는다 — 알림을 원하지 않는 사람에게까지
  // 주소를 내라고 할 이유가 없고, 안 적었으면 안 받겠다는 뜻으로 본다.
  //
  // true 로 바꾸면 이름·부서와 함께 이메일도 있어야 예약할 수 있게 된다.
  requireEmail: false,
